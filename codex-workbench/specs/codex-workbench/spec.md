...
- 摘要预览只来自 PreCompact 已生成的本地 context card；过滤插件推荐、内部目标封装、heartbeat 等系统注入内容，单项长度受限，不把完整 transcript 写入 operation ledger。
...
- **AC-028**：上下文预览和工作流解释不得写入完整 transcript、完整 patch、完整 prompt、内部目标封装、heartbeat、插件推荐或敏感值；摘要单项有明确长度上限。
