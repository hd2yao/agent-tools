...
- 额度字段：普通消耗和单独的刷新时间变化只更新观察基线，不写日志；正常/官方/本地恢复、额度耗尽或解除及重置次数变化才写入日志。
...
- 工作流文件变更不能只显示 fingerprint；Automation 优先解析名称、状态、计划、投递目标和安全能力标签，生成一到三条可读改动摘要。
- 列表摘要回答“具体调整了什么”；fingerprint 下沉为技术证据，不作为主要说明。
- 全局工作流事件区分“修改来源对话”和“运行/投递目标对话”，并明确显示“全局工作流”作用域；两类对话不得混为一个“相关对话”。
- 修改来源通过变更时间附近的本地结构化工具调用证据关联；找到证据时标为已核实，找不到时明确显示“未定位修改来源对话”。
- 对已有且仍有本地证据的 Automation 事件执行幂等回填，使旧日志也获得可读摘要和对话归属。
- 不把完整 Automation prompt、完整对话正文或工具调用参数写入 operation ledger；只保存安全标签、字段差异、线程 ID/标题、项目和证据路径。

### V1.3 Skill、Hook 与 Automation 变更说明

- Skill 与 Hook 和 Automation 使用同一条“语义快照 -> 安全差异 -> 可读摘要”链路；不能再仅记录文件名和 fingerprint。
- Skill 快照只保留用途说明、公开章节与稳定能力标签；Hook 快照只保留模块职责、公开入口与稳定能力标签，不保存正文或完整代码。
- 有新旧快照时，列表和详情明确展示“用途调整 / 新增能力 / 移除能力”；新增文件展示其“用途 / 主要能力”。
- 历史事件缺少旧快照时，允许展示可核实的“更新后职责 / 更新后包含”，但必须同时说明无法确认这些能力是否全部由本次更新新增。
- Automation 历史证据解析兼容 `id: cfg.id`、`prompt: updatedPrompt` 等局部变量写法；仍须以事件时间窗口、Automation 更新调用与可核实 ID 三重约束避免误配。
- 对已有通用文案事件执行幂等 revision 回填，使时间轴无需等待下一次文件变更即可获得说明。

### V1.4 压缩摘要与全类型变更解释

- 上下文压缩事件不能只说明“摘要卡片已生成”；详情首区展示压缩后实际保留的大致内容，包括最近有效用户要求与一至两条压缩前进展，并允许打开完整摘要卡片。
- 摘要预览只来自 PreCompact 已生成的本地 context card；过滤插件推荐、内部目标封装、heartbeat、AGENTS 指令和截图附件清单等非对话正文，单项长度受限，不把完整 transcript 写入 operation ledger。
- “具体做了什么”是所有受控工作流事件的统一门槛，不再只覆盖 Skill、Hook、Automation；全局规则、Codex 配置和 Plugin 的新增/更新也必须展示规则、设置、声明、能力或模块差异。
- 有结构化 `apply_patch` 证据时，从事件附近的唯一工具调用中提取目标文件的安全增删项，并关联修改来源对话；不保存完整 patch 或工具调用正文。
- 对已经写入台账的“内容已调整 / 实现内容已调整 / 已生成摘要卡片”等通用事件执行幂等 revision 回填；无精确证据时必须明确缺少的证据，不能把通用文案当作改动说明。
...
- **AC-017**：Automation 变更的列表摘要至少包含一项可读改动；详情按条列出结构化变化，fingerprint 仅出现在技术证据区。
- **AC-018**：具备本地工具调用证据时，工作流事件显示真实“修改来源对话”和项目；Automation 存在 `target_thread_id` 时另行显示“投递目标对话”。
- **AC-019**：无法定位来源或无法安全解释内容时必须显示明确降级文案，不能伪造归属或用空白替代。
- **AC-020**：11:04 的 `codex` Automation 历史事件可幂等回填为可读摘要，并关联来源对话 `019f44a5-9a72-7c22-881c-95a3edab1acf` 与目标对话 `019f3b58-217f-74b1-923e-ae10bf0e9ad0`。
- **AC-021**：Skill、Hook 和 Automation 的新增/更新事件在列表中至少包含一项可读职责或改动，详情不再只有文件名与 fingerprint。
- **AC-022**：有新旧语义快照时，详情分别显示用途、能力或接口的新增/移除；完整 Skill、Hook、Automation 正文不进入 operation ledger。
- **AC-023**：缺少旧版本证据时，详情明确标注“更新后职责”以及证据边界，不得把当前能力虚构为本次新增。
- **AC-024**：截图中的 11:07–11:19 Skill、Hook、Automation 通用文案事件可幂等回填；其中变量式 Automation 更新调用不得因 `cfg.id` 或 `updatedPrompt` 被漏掉。
- **AC-025**：上下文压缩事件的详情首区显示“压缩后摘要”，至少包含最近有效用户要求和一条压缩前进展；列表摘要直接概括保留内容，并可打开完整 context card。
- **AC-026**：全局规则、Codex 配置、Plugin、Skill、Hook、Automation 的新增/更新事件均不得只显示名称、fingerprint、“内容已调整”或“实现内容已调整”；列表或详情至少包含一项具体变化。
- **AC-027**：具备唯一结构化 patch 或精确 Git 前后版本时，历史工作流事件可幂等回填具体增删项与修改来源对话；截图中的全局规则事件必须显示实际新增规则。
- **AC-028**：上下文预览和工作流解释不得写入完整 transcript、完整 patch、完整 prompt、内部目标封装、heartbeat、插件推荐、AGENTS 指令、截图附件清单或敏感值；摘要单项有明确长度上限。
